...
class tree(Slide):
    ...
    def do_step_recursion(self, original_sq, steps, width, angle, angle1, group, animate):
        if steps == self.n:
            return
        UR = original_sq.get_center() + np.array(
            [width / sqrt(2) * cos(angle + pi / 4), width / sqrt(2) * sin(angle + pi / 4), 0])
        current_scale = sin(angle1)
        square1 = original_sq.copy().scale(sqrt(1 - current_scale * current_scale), about_point=UR).rotate(
            -angle1 - pi / 2, about_point=UR)
        if animate: self.play(TransformFromCopy(original_sq, square1), run_time=0.5 / steps)
        self.do_step_recursion(square1, steps + 1, width * sqrt(1 - current_scale * current_scale), angle - angle1,
                               angle1, group, animate)
        group.add(square1)
        UL = original_sq.get_center() + np.array(
            [width / sqrt(2) * cos(angle + pi - pi / 4), width / sqrt(2) * sin(angle + pi - pi / 4), 0])
        square2 = original_sq.copy().scale(current_scale, about_point=UL).rotate((pi / 2 - angle1) + pi / 2,
                                                                                 about_point=UL)
        if animate: self.play(TransformFromCopy(original_sq, square2), run_time=0.5 / steps)
        group.add(square2)
        self.do_step_recursion(square2, steps + 1, width * current_scale, angle + (pi / 2 - angle1), angle1, group,
                               animate)
